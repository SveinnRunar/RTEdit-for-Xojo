#tag DesktopWindow
Begin DesktopWindow windowChatbotDemo
   Backdrop        =   0
   ...
   MinimumWidth    =   400
   Resizeable      =   True
   Title           =   "RTEdit - Chatbot"
   Type            =   0
   Visible         =   True
   Width           =   714
   Begin DesktopButton buttonGetStyledResponse
      AllowAutoDeactivate=   True
      Bold            =   False
      Cancel          =   False
      Caption         =   "Demo a styled response"
      Default         =   False
      Enabled         =   False
      FontName        =   "System"
      FontSize        =   0.0
   ...
   Begin RTEdit REdit
      ...
      ColorBackgroundSelection=   &cFFFDC200
...
#tag WindowCode
	...
	#tag Method, Flags = &h1
		Protected Sub SetupChat()
		  ...
		  buttonGetStyledResponse.Enabled = True
...
#tag Events buttonGetStyledResponse
	#tag Event
		Sub Pressed()
		  txtPrompt.Text = "Write a small lorum ipsum text, that showcase all the different font styling that a chatgpt response can contain, including Bold, Italic, Code, Code-block, Strike-through, Bulletin, Numbered list, Heading 1, Heading 2, Heading 3  and blockquote."
		  AskChatBot
...
#tag Events txtPrompt
	...
	#tag Event
		Sub TextChanged()
		  if me.Text.Length > 6 then buttonAsk.Enabled = True else buttonAsk.Enabled = False
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events buttonAsk
	#tag Event
		Sub Pressed()
		  AskChatBot
		End Sub
